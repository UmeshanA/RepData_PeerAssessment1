...
Now that the data is in our data frame, we can tidy up the data before
...
To calculate the mean total number of steps taken per day, we first
...
Now that we have the total number of steps taken per day, the next step
is to plot a histogram of the total number of steps taken per day.
...
![](PA1_template_files/figure-markdown_strict/total%20number%20of%20steps%20histogram-1.png)

The final step is to calculate the mean and median of the total number
...
Hence we can see that the mean number of steps taken per day is
1.076618910^{4} and the median number of steps taken per day is
1.076510^{4}.

Average Daily Activity Pattern
------------------------------

Before plotting the average daily activity pattern, we will first
calculate the steps taken per interval, and aggregate that data:
...
![](PA1_template_files/figure-markdown_strict/average%20daily%20activity%20pattern%20line%20graph-1.png)

To calculate which 5-minute interval contains the maximum number of
steps across all the days in the dataset, we run the following code
chunk:
...
In order to impute missing values, we first need to figure out how many
missing values are present in the data set.
...
We can see that the number of missing values in the data set is 2304.
Now that we have this value, we can come up with a strategy to fill up
these slots. In this case, the mean for that particular interval will be
used to impute the missing values. A new function na\_impute was used to
do this:
...
Using this function, a new data frame, activityData\_fill was created,
...
Now, we check to see if there are any missing values after calling
na\_impute:
...
We can see that there are no more missing values, after running the
na\_impute function.
...
![](PA1_template_files/figure-markdown_strict/histogram%20after%20impute-1.png)

Furthermore, the new mean and median for total number of steps taken per
day is calculated:

    steps_mean_fill <- mean(daily_fill_steps$steps, na.rm = TRUE)
    steps_median_fill <- median(daily_fill_steps$steps, na.rm = TRUE)

The new mean after imputing missing values is 1.076618910^{4}, while the
new median after imputing missing values is 1.076618910^{4}.
...
After the impute, we see that the mean and the median are the same; this
is due to the fact that we used the mean value to impute the missing
values.

Differences in activity patterns between weekdays and weekends
--------------------------------------------------------------

For this analysis, the filled in data set, activityData\_fill was used.

First, a new factor variable with two levels, "weekday" and "weekend"
were created, in order to show whether a given day is a weekday or a
weekend day:
...
Now that this has been done, a panel plot containing a time series plot,
of the 5 minute interval (x axis) and the average number of steps taken,
averaged across all weekdays or weekends (y axis):
...
![](PA1_template_files/figure-markdown_strict/panel%20plot%20weekdays%20weekends-1.png)
